...
class OrchestratorEvaluator:
    ...
    def test_scenario(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Evaluación 4: Eficiencia temporal (más flexible, considera rate limiting y timeouts)
        ...
        # Para un escenario típico (5 turnos, 3 agentes, ~8-10 llamadas LLM):
        #   - Rate limiting mínimo: ~50s
        ...
        time_acceptable = execution_time <= THRESHOLDS['orchestrator'][
            'avg_time_max'] * 1.25  # 25% más de tolerancia (hasta 150s)
        ...
        # Evaluación 6: Calidad de recomendaciones (LLM Judge)
        ...
        test_result = {
            ...
            # Lógica mejorada: éxito si cumple criterios principales O si tiene buena calidad
            "overall_success": (
                    flow_success and
                    products_ok and
                    (
                        # Opción 1: Tiempo OK y relevancia básica
                            (time_ok and (keyword_relevance >= 0.3 if relevance_keywords else True)) or
                            # Opción 2: Tiempo aceptable Y buena calidad LLM
                            (time_acceptable and llm_quality_ok) or
                            # Opción 3: Tiempo aceptable Y alta relevancia de keywords
        ...
        print(f"      - Tiempo ejecución: {execution_time:.2f}s (incluye rate limiting y timeouts)")
        if execution_time > 100:
            print(f"      ⚠️  Tiempo alto - posiblemente incluye timeouts de LLM (30s por intento)")
    ...
    def run_all_tests(self, scenarios: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        print(f"   - Tiempo/Calidad: Flexible (tiempo ≤120s O tiempo ≤150s + calidad LLM ≥6.0)")
        print(f"\n⏱️  Nota sobre tiempos:")
        print(f"   - El tiempo incluye: rate limiting (~5s/request) + timeouts LLM (30s/intento)")
        print(f"   - Tiempo base esperado: ~70-80s para escenario típico")
